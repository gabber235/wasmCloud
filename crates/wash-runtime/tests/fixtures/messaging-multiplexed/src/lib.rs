...
async fn run() -> Result<Vec<u8>, String> {
    bindings::team_a::publish(BrokerMessage {
        subject: "team.events".to_string(),
        body: b"from-a".to_vec(),
        reply_to: None,
    })
    .await?;
    bindings::team_b::publish(BrokerMessage {
        subject: "team.events".to_string(),
        body: b"from-b".to_vec(),
        reply_to: None,
    })
    .await?;
    let reply = bindings::team_a::request("team-a.rpc".to_string(), b"ping".to_vec(), 5_000).await?;
    Ok(reply.body)
}
